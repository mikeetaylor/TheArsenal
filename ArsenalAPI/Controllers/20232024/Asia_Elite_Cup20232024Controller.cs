...
{
    public class Asia_Elite_Cup20232024Controller : ApiController
    {
        // GET: api/Asia_Elite_Cup
        public HttpResponseMessage Get()
        {
            string query = @"
                EXEC SelectAsiaEliteCupData
            ";
        ...
        public string Post(Roster ros)
        {
            try
            {
                DataTable table = new DataTable();
                string query = @"
                          insert into dbo.Asia_Elite_Cup Values (
                                  '" + ros.Position + @"',
                                  '" + ros.LastName + @"',
                                  '" + ros.FirstName + @"',
                                  '" + ros.Appearances + @"',
        ...
        public string Delete(int id)
        {
            try
            {
                DataTable table = new DataTable();
                string query = @"
                          delete from dbo.Asia_Elite_Cup where PlayerID = " + id;
