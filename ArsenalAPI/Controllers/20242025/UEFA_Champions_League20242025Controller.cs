...
{
    public class UEFA_Champions_League20242025Controller : ApiController
    {
        // GET: api/UEFA_Champions_League
        public HttpResponseMessage Get()
        {
            string query = @"
                SelectUEFAChampionsLeagueData
            ";
            DataTable table = new DataTable();
            using (var con = new SqlConnection(ConfigurationManager.
                ConnectionStrings["Arsenal20242025DB"].ConnectionString))
        ...
        public string Post(Roster ros)
        {
            try
            {
                DataTable table = new DataTable();
                string query = @"
                          insert into dbo.UEFA_Champions_League Values (
                                  '" + ros.Position + @"',
                                  '" + ros.LastName + @"',
                                  '" + ros.FirstName + @"',
                                  '" + ros.Appearances + @"',
                ...
                using (var con = new SqlConnection(ConfigurationManager.ConnectionStrings["Arsenal20242025DB"].ConnectionString))
                using (var cmd = new SqlCommand(query, con))
                using (var da = new SqlDataAdapter(cmd))
                {
                    cmd.CommandType = CommandType.Text;
        ...
        public string Put(Roster ros)
        {
            try
            {
                DataTable table = new DataTable();
                string query = @"
                          update dbo.UEFA_Champions_League set 
        ...
        public string Delete(int id)
        {
            try
            {
                DataTable table = new DataTable();
                string query = @"
                          delete from dbo.UEFA_Champions_League where PlayerID = " + id;
